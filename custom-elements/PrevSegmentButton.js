class PrevSegmentButton extends globalThis.HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!(playerElement instanceof HTMLElement)) {
      console.error(
        "beyondwords-prev-segment-button must be used within a beyondwords-player",
      );
      return;
    }
    if (!playerElement.instance) {
      console.error(
        "beyondwords-prev-segment-button created with uninitialized beyondwords-player",
      );
      return;
    }
  ...
  #updateDOM = () => {
    this.#buttonElement?.setAttribute(
      "aria-label",
      window.BeyondWords.translate("previousSegment"),
    );
  };
